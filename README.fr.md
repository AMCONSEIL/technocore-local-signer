...
Compte GitHub de publication : [AMCONSEIL](https://github.com/AMCONSEIL). Voir [IDENTITY.fr.md](IDENTITY.fr.md) pour distinguer cette attribution de la preuve signée liant une version du code au DID.
...
Partager les fichiers suivis de ce dépôt : sources, tests, lanceur, documentation anglaise/française, licence, requirements.txt et preuve publique d’attribution. La preuve est extraite du seul message public concerné ; les reçus bruts restent exclus. Exclure `config.local.json`, `recus`, tout PEM, toute seed, les environnements Python, caches et données personnelles. Le paquet de démonstration doit utiliser des données publiques ou synthétiques explicitement identifiées.
